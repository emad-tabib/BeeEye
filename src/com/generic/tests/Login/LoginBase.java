...
public class LoginBase extends SelTestCase {
	...
	public void LoginRegressionTest(String caseId, String runTest, String desc, String proprties, String userName, String password, String validationMsg) {
		...
		setTestCaseReportName(SheetVariables.LoginCaseId);
...
//		List<String> webPlants = new ArrayList<String>();
		...
		try {
			...
			// Step 1 do log-in
			Testlogs.get().debug(
					"Login username/password is: " + userName + " " + password);
			SignIn.fillLoginFormAndClickSubmit(userName, password);
			if (!SignIn.checkUserAccount()) {
				sassert().assertTrue(validationMsg.contains(SignIn.getErrorMessage()), "Error Message is not displayed as expected");
